...
function createProductRoutes(storage: IStorage) {
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      // Get admin settings to check if product approval is required
      let approvalRequired = true;
      try {
        const settings = await storage.getAppSettings('productApprovalRequired');
        approvalRequired = settings !== false; // Default to true if setting doesn't exist
      } catch (error) {
        console.error('Error checking product approval settings:', error);
      }

      // Return the product with approval status information
      res.status(201).json({
        ...product,
        approvalRequired,
        message: approvalRequired
          ? 'Your product has been submitted and is pending approval by an admin.'
