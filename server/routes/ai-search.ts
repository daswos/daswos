...
export function setupAiSearchRoutes(router: Router, storage: IStorage) {
  ...
  router.get('/api/ai-search', async (req: Request, res: Response) => {
    try {
      ...
      const aiResponse = await getAiResponse(query as string, searchType, storage);
...
async function getAiResponse(query: string, type: string, storage: IStorage) {
  try {
    ...
    try {
      if (type === 'information') {
        ...
        for (const keyword of productKeywords) {
          ...
          if (keyword.length >= 3) {
            const products = await storage.getProducts('safesphere', keyword);
        ...
        if (relatedContent.length === 0) {
          relatedContent = await storage.getProducts('safesphere', query as string);
        }
      }
    } catch (error) {
