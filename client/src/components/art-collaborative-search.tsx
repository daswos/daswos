...
import { Users, Search, Plus, Clock, Star, Filter } from 'lucide-react';
...
const ArtCollaborativeSearch: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleCreateNew = () => {
    ...
    navigate('/collaborative-search/create?category=art');
  };
  ...
  return (
    <Card className="w-full shadow-sm">
      <CardHeader className="bg-[#FF6B6B15]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Users className="h-5 w-5 text-[#FF6B6B]" />
            <CardTitle className="text-lg">Art & Paintings Collaborative Research</CardTitle>
          </div>
          <Button 
            onClick={handleCreateNew}
            className="bg-[#FF6B6B] hover:bg-[#FF5A5A] text-white"
            size="sm"
          >
      ...
      <CardContent className="p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {topics.map(topic => (
            <Button
              ...
              className={selectedTopic === topic.id ? "bg-[#FF6B6B] hover:bg-[#FF5A5A]" : ""}
            >
              {topic.name}
            </Button>
          ))}
        ...
        {filteredSearches.length === 0 ? (
          <div className="text-center py-8 border rounded-lg bg-gray-50">
            ...
            <Button 
              className="mt-4 bg-[#FF6B6B] hover:bg-[#FF5A5A] text-white"
              onClick={handleCreateNew}
            >
      ...
      <CardFooter className="bg-gray-50 p-4 flex justify-between">
        ...
        <Link href="/collaborative-search">
          <Button variant="link" className="text-sm p-0 h-auto text-[#FF6B6B]">
...
export default ArtCollaborativeSearch;
